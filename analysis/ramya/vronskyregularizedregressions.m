function [results, times] = vronskyregularizedregressions()
	...
	for i = 3:28
		...
		tic;
		Weights = ((inv(X' * X) * X') * Y);
		%Predfun is a different file with OLS solution to pass in.
		mse = crossval('mse', X, Y, 'Predfun', @predfun, 'kfold', 10);
		results(i - 2, 1) = mse;
		runTime = toc;
